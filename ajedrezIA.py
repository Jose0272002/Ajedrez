    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if player_turn:  # Verifica si es el turno del jugador
                    node = Find_Node(pygame.mouse.get_pos(), WIDTH)
                    if selected_node is None:
                        # Primer clic: guarda la casilla seleccionada
                        selected_node = node
                    else:
                        # Segundo clic: verifica si es el mismo que el primer clic
                        if node == selected_node:
                            # Ignora el segundo clic en la misma casilla
                            selected_node = None
                            continue
                        else:
                            movement = selected_node + node
                            selected_node = None  # Reinicia la casilla seleccionada

                            if not chess.Move.from_uci(movement) in board.legal_moves:
                                print("Movimiento no válido")
                            else:
                                board.push(chess.Move.from_uci(movement))
                                player_turn = False  # Cambia al turno de la máquina
        ...
        if not player_turn:
            ...
            try:
                ...
                board.push(chess.Move.from_uci(movement))
                movement = ""
                player_turn = True  # Cambia de nuevo al turno del jugador
            except Exception as e:
                print("Error en el movimiento de la máquina:", e)
                # Maneja el error (por ejemplo, muestra un mensaje de error) o continúa el juego
